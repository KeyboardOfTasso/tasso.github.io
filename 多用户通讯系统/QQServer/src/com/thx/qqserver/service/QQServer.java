...
public class QQServer {
    ...
    static { //在静态代码块，初始化 validUsers
        ...
        validUsers.put("艾雅法拉", new User("艾雅法拉", "123456"));
        validUsers.put("阿米娅", new User("阿米娅", "123456"));
        validUsers.put("博士", new User("博士", "123456"));
    ...
    public QQServer() {
        ...
        try {
            ...
            while (true) { //当和某个客户端连接后，会继续监听, 因此while
                ...
                //验证用户 方法
                if (checkUser(u.getId(), u.getPassword())) {//登录通过
                    message.setMessageType(MessageType.MESSAGE_LOGIN_SUCCEED);
                    //将message对象回复客户端
                    oos.writeObject(message);
                    //创建一个线程，和客户端保持通信, 该线程需要持有socket对象
                    ServerConnectClientThread serverConnectClientThread =
                            new ServerConnectClientThread(socket, u.getId());
                    //启动该线程
                    serverConnectClientThread.start();
                    //把该线程对象，放入到一个集合中，进行管理.
                    ManageClientThreads.addClientThread(u.getId(), serverConnectClientThread);

                } else { // 登录失败
                    System.out.println("用户 id=" + u.getId() + " pwd=" + u.getPassword() + " 验证失败");
